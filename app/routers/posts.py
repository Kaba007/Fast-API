...
from ..schemas import UserCreate,UserOut,PostResponse,PostCreate, PostOUT,Postdetails #nějka se tomu nechtělo
...
def get_post(id: int, db: Session = Depends(get_db)):
    # Načtěte příspěvek a jeho komentáře
    post = db.query(models.Post).options(joinedload(models.Post.comments)).filter(models.Post.id == id).first()

    if post is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"post with id: {id} was not found")

    # Získání počtu hlasů
    votes_count = db.query(func.count(models.Vote.post_id)).filter(models.Vote.post_id == id).scalar()

    # Vytvoření response
    return {
        "Post": post,  # Předpokládá, že máte definovaný PostResponse model nebo můžete post přímo vrátit
        "votes": votes_count,
